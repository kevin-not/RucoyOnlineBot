...
def image_to_cv(img, alpha=None):
    # Load the image initially in color (RGB)
    res_ = resource_path(img)
    temp_img = cv2.imread(res_)

    # Check if the image has 4 channels (RGBA)
    if alpha != None:
        # Load with IMREAD_UNCHANGED to keep alpha channel if present
        img_with_alpha = cv2.imread(res_, cv2.IMREAD_UNCHANGED)
        return img_with_alpha
    else:
        # Return the RGB image if it's already in the correct format
        return temp_img
...
class ScreenImage:
    ...
    def find_on_screen(self, small_image, threshold=0.7, mask=None):
        h, w = small_image.shape[:-1]
        if mask is not None:
            res = cv2.matchTemplate(self.img_rgb, small_image, cv2.TM_CCOEFF_NORMED, mask=mask)
        else:
